    ...
    if (searchType === 'frames' || searchType === 'all') {
          ...
          $addFields: {
            ...
            title_relevance: {
              $cond: [
                ...
                {
                  $cond: [
                    { $regexMatch: { input: '$desc', regex: searchRegex } },
                    5,
                    1 
                  ]
                }
      ...
      results.frames = {
        data: frames.map(frame => ({
          ...
          total_likes: frame.total_likes || frame.like_count?.length || 0,
          total_uses: frame.total_uses || frame.use_count?.length || 0,
          ...
          relevance_score: frame.title_relevance,
          user: {
            id: frame.user[0]._id,
            name: frame.user[0].name,
            username: frame.user[0].username,
            image_profile: getDisplayProfileImage(frame.user[0], req),
            role: frame.user[0].role
          },
