...
class CleanupScheduler {
  constructor() {
    ...
    this.stats = {
      ...
      total_files_deleted: 0,const cron = require('node-cron');
const photoCleanupService = require('../services/photoCleanupService');
...
class CleanupScheduler {
    ...
    this.job = cron.schedule(cronExpression, async () => {
      ...
      try {
        ...
        if (cleanupResult.success) {
          ...
          console.log(`   🗃️ DB records deleted: ${cleanupResult.deleted_records}`);
          ...
          if (cleanupResult.failed_records > 0 || cleanupResult.failed_files > 0) {
            console.warn(`⚠️ Some operations failed during cleanup - check logs above`);
          }
  ...
  async runManualCleanup() {
    ...
    try {
      ...
      // Don't count manual runs in automated stats
  ...
  async runOrphanedCleanup() {
    ...
    try {
      this.isRunning = true;
      console.log('🗃️ Manual orphaned files cleanup triggered');
  ...
  logStatus() {
    ...
    console.log(`   🗃️ Total records deleted: ${status.statistics.total_records_deleted}`);
